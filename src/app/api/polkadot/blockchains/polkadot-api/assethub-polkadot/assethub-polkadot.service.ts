...
export class AssethubPolkadotService extends PolkadotApiService {
  ...
  async getBalance(api: Api<AssethubPolkadot>, token: Token, publicKey: string): Promise<Balance> {
  ...
  watchBalance(api: Api<AssethubPolkadot>, token: Token, publicKey: string): Observable<Balance> {
    return new Observable<Balance>(subscriber => {
      const subscriptions: any[] = [];
